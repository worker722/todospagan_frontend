...
import { consumer_insert } from "../../_redux/authCrud";
...
const RegulatoryData = (props) => {
    ...
    const { consumer_regulatory, consumer_document, user_info, personal_reference, labor_data } = useSelector((state) => state.auth);
    ...
    useEffect(() => {
        if(consumer_regulatory) {
            setExceed(consumer_regulatory?consumer_regulatory['is_exceed']:false);
            setShowPolitical(consumer_regulatory?consumer_regulatory['is_fir_politician']:false);
            setShowRelationShip(consumer_regulatory?consumer_regulatory['is_sec_plitician']:false);
        }
    }, [])
    return (
        <>
            <Formik
                ...
                initialValues={consumer_regulatory?consumer_regulatory:initCustomer}
                ...
                onSubmit={(values) => {
                    ...
                    props.setConsumerRegulatory(values);
                    consumer_insert(user_info, personal_reference, labor_data, consumer_document, values)
                    .then(({ data }) => {
                        console.log(data);
                    })
                    .catch((error) => {
                ...
                {({ handleSubmit }) => (
                    <>
                        ...
                        <Modal.Footer>
                            <button
                                type="button"
                                onClick={() => setContentName("UploadDocument")}
...
export default injectIntl(connect(null, auth.actions)(RegulatoryData));